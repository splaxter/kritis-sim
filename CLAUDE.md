...
- The jsdom `WindowsLevel` specs use 5 s waits and **flake under machine load** — a parallel `npm run dev` is enough to fail 3–11 of them with `Test timed out in 5000ms`. Green again once the load is gone; don't chase it as a regression.
