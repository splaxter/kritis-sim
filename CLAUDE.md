...
- **Two guards answer "does the game ever demand what it never showed?"** — the question behind two escaped defects, both found by a human playing. `engine/anforderungen.ts` derives what a level REQUIRES from its win condition (`stateGoals`, `solutions[].commands`), never from its hints: a requirement that vanishes when its explanation is deleted is the exact bug being hunted. `engine/wissensbilanz.ts` then walks each FIXED-ORDER route and fails the build on a requirement no candidate of which appears anywhere — currently zero across 17 routes — and on any goal kind the derivation cannot map (visible-unproven beats silently-clean). Fixed-order means: the learning path is NOT one chain (tracks are independently unlockable after Foundations, so it is one route per track), optional lessons teach nothing the later levels may assume, and only single-candidate requirements count as learned — crediting all twelve write-candidates once made an unannounced `sha256sum` look known. A pair-level ratchet (route|level|requirement) covers requirements explained only in a hint; counts per route let two regressions through. `engine/sollpfadFahrer.ts` + `abschreibDurchstich.test.ts` derive the visible path for all 87 terminal levels and run it against the real shell — the driver must tick `scheduleDrip` and STOP at the first success, because the confirming Enter resets `solved` and four levels were mis-filed as unsolvable. All three sets (solvable, incomplete, underivable) plus the registry total are asserted by name, or a vanishing level slips through every assertion. Neither guard replaces a beginner: they prove reachability, not how it feels.
