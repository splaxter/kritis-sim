...
- **Two guards answer "does the game ever demand what it never showed?"** — the question behind two consecutive escaped defects, both found by a human playing, not by a test. `engine/wissensbilanz.ts` walks each FIXED-ORDER route (guided onboarding, learning path, each campaign) and accumulates the commands the game has demonstrated; a level requiring a command that appears **nowhere** — not in the task, not in a hint, not in a scripted line — fails the build. Currently zero across all routes. A softer ratchet counts commands explained only in a HINT and not in the task (10 today), because taking four hints to make the first move is not learning. `engine/abschreibDurchstich.ts`+`sollpfad.ts` derive the visible path for all 87 terminal levels (scripted beats or backticked hint lines) and run it against the real shell: 54 solve by copying alone, 23 have paths the hints deliberately leave incomplete, 10 cannot be derived — all three sets are named lists, so a level silently leaving one is a failure. Simulation modes have no "before" (`selectNextEvent` picks uniformly), so only the copy-through covers them. Neither replaces a real beginner: they prove reachability, not how it feels.
