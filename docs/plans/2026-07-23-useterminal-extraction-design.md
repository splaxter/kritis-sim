...
- **Aufräum-Punkte (nicht verhaltensrelevant):** Die toten `pendingCmdName`-Felder und der ungenutzte `renderInput`-Wert-Import wurden vor dem Extraktions-Merge entfernt. Der redundante `TerminalSessionDeps.onPartialSolution` (Session emittiert `showPartial`, der Adapter mappt es) und die tote `showPage`-Effekt-Variante wurden im separaten Cleanup-Branch `chore/terminal-effects-cleanup` entfernt.
- **Eine bewusste, benigne Verhaltens-Nuance:** Der Footer-`[?]`-Button (`handleHintRequest`) zeichnet die aktuelle Eingabezeile neu (`renderInput`), während das Original einen nackten Prompt schrieb. Das weicht nur ab, wenn der Spieler bei **teilweise getipptem** Befehl auf `[?]` klickt — dann bleibt der getippte Text sichtbar erhalten (korrekter; behebt eine latente visuelle Desynchronisation des Originals). Der Normalfall (leere Zeile, „ich stecke fest") ist identisch. Der Idle-💡-Hint dagegen wurde exakt ans Original angeglichen (`handleIdleHint`).
