...
- **Offene Aufräum-Punkte (nicht verhaltensrelevant, als Follow-up vertagt):** `TerminalSessionDeps.onPartialSolution` wird von der Session nicht aufgerufen (sie emittiert `showPartial`; der Adapter mappt es) — der Dep ist redundant. `showPage` ist toter Code (nie emittiert). Die toten `pendingCmdName`-Felder und der ungenutzte `renderInput`-Wert-Import wurden vor dem Merge entfernt.
