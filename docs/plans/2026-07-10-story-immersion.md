...
> **✅ AUFGELÖST (2026-07-10, nach Menü-IA-Merge): kein Code nötig.** Nach dem Merge von Menü-IA + Immersion in `main` wurde der Bug empirisch neu reproduziert (Browser, instrumentierter Screen-Übergangs-Logger): 15× Enter aus einem Event und 12× Enter aus einem ResultScreen → **nie** Menü/Act-Break/Picker (`hitMenu: false`), Fluss bleibt in Event/Result. Der Menü-Sprung ist durch zwei bereits gemergte, unabhängige Änderungen verhindert: (1) **Typewriter-Enter-Skip (Task 2)** — ein Enter-Burst auf einem frischen Event löst nur „Text überspringen" aus (bevor React `done` neu rendert), also lässt sich die Story nicht durchhämmern; (2) die `storyEnding`/Act-Break-Screens haben **keinen aktiven Enter-Handler** und keinen autofokussierten Menü-Button, der Menü-keydown ist auf `phase === 'menu'` gated, und GameScreen (mit dem Result-Enter-Handler) ist während `storyEnding` unmounted. Der geplante `useInputGuard` würde einen nicht-reproduzierbaren Bug „fixen" (YAGNI) → **nicht umgesetzt.** Rest-Notiz: bei `prefers-reduced-motion` entfällt Mitigation (1), aber (2) trägt weiterhin — kein Menü-Sprung möglich. Die folgende Spezifikation bleibt als Referenz, falls das Verhalten je zurückkehrt.
