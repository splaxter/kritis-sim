...
# Telnet bestätigt: Port 8443 ist dicht`,
      ...
      hints: [
        'Tipp: Marco sagt "die Firewall-Regel existiert nicht" — aber prüf das selbst. Erreichst du den Ziel-Port 8443 auf dem Server überhaupt?',
        'Tipp: PowerShell hat ein Cmdlet, das gezielt einen einzelnen TCP-Port testet (nicht nur ping). Teste damit 8443 — und zum Vergleich 443.',
        'Tipp: Konkret: Test-NetConnection ziel-server.warm.local -Port 8443 (alternativ telnet ziel-server.warm.local 8443).',
      ],
    },
  },
  {
...
# Das Netz ist von hier aus nicht erreichbar (noch nicht...)`,
      ...
      hints: [
        'Tipp: Marco behauptet, die Regel "existiert nicht" — verschaff dir selbst einen Überblick über die aktiven Firewall-Regeln auf diesem Management-Host.',
        'Tipp: Eine Regel verweist auf ein 10.42er Netz, das gar nicht zu WARM gehört. Wem gehört diese IP-Range — und wer hat die Regel laut "Created by"-Spalte angelegt?',
        'Tipp: Konkret: zeig die Regeln mit "show firewall rules" und prüf den Eigentümer der Range mit whois 10.42.0.0.',
      ],
    },
  },
  {
...
# Workaround aktiv! Internet sollte jetzt wieder gehen.`,
      ...
      hints: [
        'Tipp: "Leitung tot" behauptet der Kunde — beweis oder widerleg das. Teste zuerst die reine Verbindung zu einer IP statt zu einem Namen.',
        'Tipp: Wenn der Ping auf eine IP durchgeht, aber auf google.de nicht — dann ist es nicht die Leitung. Was übersetzt Namen in IPs?',
        'Tipp: Konkret: ping 8.8.8.8 testet die Leitung, nslookup google.de prüft ob DNS funktioniert.',
      ],
    },
  },
  {
...
# Aktivität sieht normal aus, keine verdächtigen Befehle.`,
      ...
      hints: [
        'Tipp: Ein Account namens admin_amse macht dich stutzig — verschaff dir einen Überblick, welche Admin-Sessions auf diesem Firewall-Host gerade aktiv sind.',
        'Tipp: Eine Session läuft seit Wochen von einer externen IP. Wem gehört die Adresse 85.214.47.123?',
        'Tipp: Konkret: zeig die Sessions mit "show sessions", prüf die externe IP mit whois 85.214.47.123, und wirf einen Blick in die Command-History des Accounts.',
      ],
    },
  },
];
